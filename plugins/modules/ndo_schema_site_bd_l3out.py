...
from ansible_collections.cisco.mso.plugins.module_utils.mso import MSOModule, mso_argument_spec, mso_reference_spec, get_template_id
...
def main():
    ...
    if state == "absent":
        if l3out_exist:
            ...
            ops.append(dict(op="remove", path="{0}/{1}/l3Outs/{2}".format(bd_path, bd, l3outs.index(l3out.get("name")))))
            ops.append(dict(op="remove", path="{0}/{1}/l3OutRefs/{2}".format(bd_path, bd, l3out_refs.index(l3out_uuid))))
        ...
        if not l3out_exist:
            ...
            l3out_refs.append(l3out_uuid)
            ops.append(dict(op="add", path="{0}/{1}/l3Outs/-".format(bd_path, bd), value=l3out['name']))
            ops.append(dict(op="add", path="{0}/{1}/l3OutRefs/-".format(bd_path, bd), value=l3out_uuid))
            if not module.check_mode:
                mso.request(mso_schema.path, method="PATCH", data=ops)
        ...
        #     l3outs.append(l3out.get("name"))
        #     l3out_refs.append(l3out_uuid)
        #     ops.append(dict(op="add", path="{0}/{1}".format(bd_path, bd), value=mso_objects.get("site_bd").details))
