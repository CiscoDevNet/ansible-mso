...
short_description: Manage contract service graph in schema templates
description:
- Manage contract service graph in schema templates on Cisco ACI Multi-Site.
- The Contract Service Graph parameter is supported on versions of MSO that are 3.3 or greater.
author:
- Akini Ross (@akinross)
options:
  schema:
  ...
  service_graph_nodes:
    ...
    suboptions:
      provider:
        description:
        - The provider connector of the node.
        required: true
        type: str
      consumer:
        description:
        - The consumer connector of the node.
        required: true
        type: str
...
def main():
    ...
    argument_spec.update(
        ...
        service_graph_nodes=dict(type='list', elements='dict', options=mso_service_graph_connector_spec()),
    ...
    module = AnsibleModule(
        ...
        required_if=[
            ...
            ['state', 'present', ['service_graph', 'service_graph_nodes']],
        ],
    )
    ...
    service_graph_nodes = module.params.get('service_graph_nodes')
    ...
    if state == 'absent':
        ...
        # Initialize "present" specific variables
        ...
        if service_graph_schema_obj:
            if len(service_graph_nodes) < len(service_graph_schema_obj.get('serviceNodes')):
                mso.fail_json(msg="Not enough service nodes defined, {0} service node(s) provided when {1} needed.".format(
                    len(service_graph_nodes), len(service_graph_schema_obj.get('serviceNodes'))))
            elif len(service_graph_nodes) > len(service_graph_schema_obj.get('serviceNodes')):
        ...
        for node_id, service_node in enumerate(service_graph_nodes, 1):
