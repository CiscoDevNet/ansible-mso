...
def main():
    ...
    if state == "present":
        ...
        if use_same_instance:
            ...
            process_policy(mso_values, mso_template, tenant_template, custom_qos_policy, "qosRef", "qosPolicies")
        else:
            ...
            if custom_qos_policy:
                ...
                if not empty_custom_qos_policy:
                    ...
                    process_policy(mso_values, mso_template, tenant_template_qos, custom_qos_policy, "qosRef", "qosPolicies")
    ...
    for protocol_name, protocol_dict in protocols_dict.items():
        if protocol_dict is not None:
            if (
                ...
                failed_attributes[protocol_name] = "key and key_id are required under {0}".format(protocol_name)
                ...
                failed_attributes[protocol_name] = "{0} must be enabled in order to use authentication, key and key_id".format(protocol_name)

    if failed_attributes:
        failure_messages = " | ".join(failed_attributes.values())
...
def process_policy(mso_values, mso_template, tenant_template, policy, policy_ref, policy_type):
