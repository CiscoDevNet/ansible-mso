...
def main():
    ...
    if state == "present":
        ...
        if mso.existing:
            ...
            append_update_ops_data(ops, proposed_payload, port_channel_attrs_path, payload)
            mso.sanitize(proposed_payload, collate=True)

        else:
            if not node:
                mso.fail_json(msg=("Missing parameter 'node' for creating a Port Channel Interface"))
            payload["interfaceDescriptions"] = format_interface_descriptions(mso, interface_descriptions, node)
